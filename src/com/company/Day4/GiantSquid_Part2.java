...
public class GiantSquid_Part2 {
    ...
    public static  void main(String[] args) {
        ...
        // Get input: Calls are stored in a String array, boards are stored in a 3D ArrayList
        for (int i = 0;;i++) {
            if (i == 0)
                    ...
                    for (int k = 0; k < boards.get(j).size(); k++) {
                        ...
                        for (int l = 0; l < boards.get(j).get(k).size(); l++) {
                            ...
                            if (boards.get(j).get(k).get(l).equals(calls[i])) {
                                ...
                                // Check Rows and Cols
                                // If not last board and found winner, remove from boards
                                if (boards.size() > 1 && (CheckRows(boards.get(j)) > 0 || CheckCols(boards.get(j)) > 0)) {
                                    boards.remove(j);
                                    if (j > 0)
                                        j--;
                                } else if (CheckRows(boards.get(j)) > 0) {
